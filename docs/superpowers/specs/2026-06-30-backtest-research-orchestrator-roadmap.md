...
- ⬜ **R3b-2 — per-гипотеза train/holdout arbiter** (extend R3a-гейта на `hypothesis.build`/`evaluateBacktest`, все cycleDepth; прошитое R3b-1 окно делает per-hypothesis T-splits консистентными) — не начат. **R3c** — multi-fold WFA → consume backtester E3.
