...
**Round-trip status (Task 7, 2026-06-15):** the live round-trip was **not run in this dev environment** — `TRADING_PLATFORM_GATEWAY_COMMAND` is unset, so no MCP gateway is available. The CLI was verified end-to-end up to the gateway boundary: `pnpm platform:validate <bundle.json>` parses the bundle, loads config, and fails cleanly (`exit 1`, `platform:validate failed: TRADING_PLATFORM_GATEWAY_COMMAND is required for mcp integration`, no hang). The final whole-branch review then caught a real byte-equivalence bug: the mapper's `canonicalJson` omitted the trailing `\n` that `trading-platform/src/research/backtest/canonical-json.ts` appends, which would have triggered `bundle_integrity_violation` on every real call. Fixed in commit `8647fc5` — the mapper now byte-matches the authoritative platform serializer for the strings-only hash payload (verified statically against the platform source). What still **requires the live round-trip to confirm** is end-to-end gateway acceptance (that no other descriptor field or 017-manifest check structurally rejects the submitted bundle). **Pending manual step:** with a configured gateway, run `pnpm platform:validate <bundle.json>` and confirm a typed `ValidationReport` (a `rejected` status from 017 manifest shortfalls is fine — see below); an `ok:false` `bundle_load_failed` / `bundle_integrity_violation` / `unsupported_contract_version` / path-traversal error indicates a structural mapper bug to fix in `toSubmittedBundle`.
