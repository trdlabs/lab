...
- `identity.strategyName` = **`bundle.manifest.id`** (реконструированный бандл уже в руках у handler'а; человекочитаемая идентичность модуля, напр. `long_oi_dump_reversal_v1`). РЕШЕНО на ревью Task 2: `profile.name` в домене НЕ существует (поле `name` в strategy-profile.ts — это `StrategyParameterSchema`, имя параметра), а `profile.id` — UUID, который платформа спроецировала бы оператору в bot_bundle.metadata. Manifest id — второй спекой-санкционированный источник; mapper принимает его обязательным входом `bundleManifestId`. `identity.side` — из профиля (направление стратегии; long_oi → 'long'; платформа принимает ТОЛЬКО long|short); `identity.params` = champion params = `params` holdout-member'а WFO (роль `holdout`, `oos: true`).
